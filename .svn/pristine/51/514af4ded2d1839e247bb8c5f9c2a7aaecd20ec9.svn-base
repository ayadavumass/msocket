...
public class GeoLoadProxyPolicy extends ProxySelectionPolicy
{
   ...
   *          of the request
   */
  public GeoLoadProxyPolicy(String proxyGroupName, GnsCredentials gnsCredentials, int numProxies, int maxTCPConn,
      int maxThroughput)
  {
  ...
  public List<InetSocketAddress> getNewProxy() throws Exception
  {
    ...
    for (int i = 0; i < guids.length(); i++)
    {
      ...
      String locationIP = gnsClient.fieldRead(locationGuid, GnsConstants.LOCATION_SERVICE_IP, guidEntry).getString(0);
