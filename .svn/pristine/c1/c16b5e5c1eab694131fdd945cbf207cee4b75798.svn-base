...
public class MServerSocketController implements Runnable
{
   ...
   */
  private static final int              proxyFailureTimeout     = 10;
  ...
  void setGNSCredential(GnsCredentials gnsCredentials)
  {
    this.gnsCredentials = gnsCredentials;
  }

  GnsCredentials getGNSCredential()
  {
  ...
  private void handleProxyFailure() throws Exception
  {
    ...
    for (int i = 0; i < vect.size(); i++)
    {
      ...
      else
      {
        ...
        try
        {
          ...
          GnsIntegration.unregisterWithGNS(mserversocket.getServerName(), badProxyAdd, gnsCredentials);
        }
        catch (Exception e)
        {
          e.printStackTrace();
    ...
    if (proxyMap.size() == 0)
    {
      ...
      GnsIntegration.registerWithGNS(mserversocket.getServerName(), retProxy, gnsCredentials);
