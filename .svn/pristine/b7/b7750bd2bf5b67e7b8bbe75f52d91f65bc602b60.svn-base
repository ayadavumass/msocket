...
public class WatchdogWatcherThread extends Thread
{
   ...
   *          inactive/failde
   */
  public WatchdogWatcherThread(UniversalGnsClient gnsClient, GuidEntry watchdogGuid, String targetGuid,
  ...
  public void run()
  {
    ...
    try
    {
      refreshFrequencyInMs = gnsClient.fieldRead(targetGuid, GnsConstants.TIME_REFRESH_INTERVAL, watchdogGuid).getLong(
          0);
  ...
  private long readRemoteTime()
  {
    try
    {
      return gnsClient.fieldRead(targetGuid, GnsConstants.CURRENT_TIME, watchdogGuid).getLong(0);
    }
    catch (Exception e)
    {
