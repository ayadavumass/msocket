...
public class MServerSocket extends ServerSocket
{
  ...
  /** Credentials to use to access the GNS */
  private GnsCredentials          gnsCredentials      = null;
  ...
  /**
   * Creates a new MServerSocket using the default GNS credentials, using no proxies,
  ...
  {
    this(serverName, GnsCredentials.getDefaultCredentials());
  }
   ...
   * @throws Exception
   */
  public MServerSocket(String serverName, GnsCredentials gnsCredentials) throws IOException
  {
    this(serverName, GnsIntegration.getDefaultProxyPolicy(), gnsCredentials, null, 0);
  }
  ...
  {
    this(serverName, proxySelection, GnsCredentials.getDefaultCredentials(), null, 0);
  }
   ...
   *           NAT
   */
  public MServerSocket(String serverName, ProxySelectionPolicy proxyPolicy, GnsCredentials gnsCredentials,
      SocketAddress endpoint, int backlog) throws IOException
  {
    initializeServerSocket(serverName, proxyPolicy, gnsCredentials, endpoint, backlog);
  }
  ...
  public void close() throws IOException
  {
    ...
    try
    {
      GnsIntegration.unregisterWithGNS(getServerName(), getGnsCredentials());
    }
    catch (Exception e)
    {
   ...
   */
  public void bind(String serverName, SocketAddress endpoint, int backlog, GnsCredentials gnsCredentials)
      throws IOException
  {
    if (isBound)
      throw new IOException("MServerSocket already bound");
    
    initializeServerSocket(serverName, new NoProxyPolicy(), gnsCredentials, endpoint, backlog);
  }
  ...
  public void bind(SocketAddress endpoint, int backlog) throws IOException
  {
	    ...
	    if(!serverAlias.equals(""))
	    {
	    	try
	    	{
	    		GnsIntegration.registerWithGNS(serverAlias, (InetSocketAddress) ss.getLocalSocketAddress(), gnsCredentials);
	    	}
	    	catch (Exception ex)
	    	{
  ...
  /**
   * Returns the gnsCredentials value.
   * 
   * @return Returns the gnsCredentials.
   */
  public GnsCredentials getGnsCredentials()
  {
    return gnsCredentials;
  }
      ...
      throws IOException
  {
    ...
    synchronized (migrateMonitor)
    {
      ...
      if (proxySelection.hasAvailableProxies())
      {
        ...
        GnsIntegration.unregisterWithGNS(serverAlias, gnsCredentials);

        for (Iterator<InetSocketAddress> iterator = proxyVector.iterator(); iterator.hasNext();)
        {
          InetSocketAddress retProxy = iterator.next();
          GnsIntegration.registerWithGNS(serverAlias, retProxy, gnsCredentials);
  ...
  private void initializeServerSocket(String serverName, ProxySelectionPolicy proxyPolicy,
	      GnsCredentials gnsCredentials, SocketAddress endpoint, int backlog) throws IOException
	{
	    ...
	    if (endpoint == null)
	    {
	      InetAddress serverAddr = returnLocalPublicIP();
	    ...
	    if (proxyVector != null)
	    {
	      ...
	      for (Iterator<InetSocketAddress> iterator = proxyVector.iterator(); iterator.hasNext();)
	      {
	        ...
	        if(firstTime)
	        {
	        	serverGUID = GnsIntegration.getGUIDOfAlias(serverAlias, gnsCredentials);
	            controller.setProxyConnObj(new ConnectionToProxyServer(serverGUID, controller));
	            firstTime = false;
	        }
	        
	        ProxyInfo proxyInfo = new ProxyInfo(retProxy.getHostName(), retProxy.getPort());
	        System.out.println("proxy host name "+retProxy.getHostName() +" port "+retProxy.getPort());
	        proxyInfo.setActive(true);
	        controller.getProxyConnObj().addProxy(proxyInfo.getProxyInfo(), proxyInfo);
	      }
	    }
   ...
   * @throws Exception
   */
  private String returnPublicIP() throws IOException
  {
   ...
   * @throws Exception
   */
  private boolean isServerBehindNAT() throws IOException
  {
   ...
   * @throws Exception
   */
  private InetAddress returnLocalPublicIP() throws IOException
  {
    Vector<InetAddress> CurrentInterfaceIPs = CommonMethods.getActiveInterfaceInetAddresses();
