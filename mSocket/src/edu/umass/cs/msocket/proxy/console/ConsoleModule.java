...
public class ConsoleModule
{
  ...
  {
    String gns = KeyPairUtils.getDefaultGnsFromPreferences();
    printString("Default GNS: " + gns + "\n");
    if (gns == null)
      return;
    GuidEntry guid = KeyPairUtils.getDefaultGuidEntryFromPreferences(gns.replace(' ', ':'));
   ...
   */
  public void setGnsClient(UniversalGnsClient gnsClient)
  {
  ...
  {
    KeyPairUtils.setDefaultGuidEntryToPreferences(gnsClient.getGnsHost() + ":" + gnsClient.getGnsPort(),
        guid.getEntityName());
    if (gnsClient != null)
    {
      accountVerified = isAccountVerified(currentGuid);
  ...
  public String getGnsHostPort()
  {
    if (gnsClient == null)
      ...
      return gnsClient.getGnsHost() + ":" + gnsClient.getGnsPort();
  }
  ...
  public boolean isAccountVerified(GuidEntry guid)
  {
    try
    {
      ...
      { // This is not an account GUID but make sure the GUID is valid
