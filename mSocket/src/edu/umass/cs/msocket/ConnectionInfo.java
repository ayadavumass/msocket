...
public class ConnectionInfo
{
    ...
    while (i < vect.size())
    {
      ...
      try
      {
        if (value.getStatus()) // only active sockets
        {
        	...
        	log.trace( "Server sending keep alive on socketId " + value.getSocketIdentifer() );
        	int mesgType;
        	if( value.getClosing() )
        	{
  ...
  private int singleSocketRead(SocketInfo socketObj) throws IOException
  {
    ...
    { // ndirect==0
      ...
      if (dmheader != null)
      {
        ...
        log.trace(getServerOrClient() + " Data message header encountered header length " + dmheader.length
            + " dmheader.sendSeq " + dmheader.sendSeq);
        ...
        {
          log.trace("Keep alive recvd from server clock" + KeepAliveStaticThread.getLocalClock());
          socketObj.setLastKeepAlive(KeepAliveStaticThread.getLocalClock());
        }
	      ...
	      try
	      {
	        ...
	        if ((MigrationType == MSocketConstants.CLIENT_MIG) && (Operation == SetupControlMessage.MIGRATE_SOCKET))
	        {
	          if (typeOfCon == MSocketConstants.CON_TO_GNSNAME)
	          {
	            ...
	            List<InetSocketAddress> socketAddressFromGNS = GnsIntegration.getSocketAddressFromGNS(serverAlias, null);
	        ...
	        switch (Operation)
	        {
	          ...
	          case SetupControlMessage.MIGRATE_SOCKET :
	          {
	            ...
	            System.out.println("setupControlRead done");
	            ...
	            System.out.println("set the new socket");
  ...
  public void internalClose()
  {
    ...
    {
      System.out.println("unregistering with mobility manager and udp controller");
